...
Requires Python 3.10 or later; the repo pins 3.13 via `.python-version`, which is the
version tested against and the one with reliable prebuilt wheels for the ML
dependencies. Avoid pre-release interpreters — `torch`, `chromadb`, and
